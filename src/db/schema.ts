import {int, mysqlTable, varchar, timestamp, boolean, uniqueIndex, text} from 'drizzle-orm/mysql-core';
...
export const userTagRelation = mysqlTable('user_tag_relation', {
  id: int('id').autoincrement().primaryKey(),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  userId: int('user_id').notNull().references(() => user.id),
  tagId: int('tag_id').notNull().references(() => tag.id),
}, (table) => ({
...
export const articleTagRelation = mysqlTable('article_tag_relation', {
  id: int('id').autoincrement().primaryKey(),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  articleId: int('article_id').notNull().references(() => article.id),
  tagId: int('tag_id').notNull().references(() => tag.id),
}, (table) => ({
